...
if (get_cookie('ck_visit_ip') != $_SERVER['REMOTE_ADDR'])
{
    ...
    $sql = " insert {$g5['visit_table']} ( vi_id, vi_ip, vi_date, vi_time, vi_referer, vi_agent, vi_browser, vi_os, vi_device ) values ( '{$vi_id}', '{$remote_addr}', '".G5_TIME_YMD."', '".G5_TIME_HIS."', '{$referer}', '{$user_agent}', '{$vi_browser}', '{$vi_os}', '{$vi_device}' ) ON DUPLICATE KEY UPDATE vi_time = '".G5_TIME_HIS."', vi_referer = '{$referer}', vi_agent = '{$user_agent}', vi_browser = '{$vi_browser}', vi_os = '{$vi_os}', vi_device = '{$vi_device}' ";

    $result = sql_query($sql, FALSE);
    // 정상으로 INSERT 되었다면 방문자 합계에 반영
    if ($result) {
        $sql = " insert {$g5['visit_sum_table']} ( vs_count, vs_date) values ( 1, '".G5_TIME_YMD."' ) ON DUPLICATE KEY UPDATE vs_count = vs_count + 1 ";
        $result = sql_query($sql, FALSE);
