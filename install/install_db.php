...
// 관리자 회원가입
$admin_hash = sql_password($admin_pass);
$sql = " insert into `{$table_prefix}member`
            set mb_id = '$admin_id',
                 mb_password = '{$admin_hash}',
                 mb_name = '$admin_name',
                 mb_nick = '$admin_name',
...
// 기본 메뉴 생성 (관리자 > 환경설정 > 메뉴설정에서 수정 가능)
$default_menus = array(
    array('10', '공지사항', '/'.G5_BBS_DIR.'/board.php?bo_table=notice', 'self', 10),
    array('20', '질문답변', '/'.G5_BBS_DIR.'/board.php?bo_table=qa', 'self', 20),
    array('30', '자유게시판', '/'.G5_BBS_DIR.'/board.php?bo_table=free', 'self', 30),
    array('40', '갤러리', '/'.G5_BBS_DIR.'/board.php?bo_table=gallery', 'self', 40)
);

foreach ($default_menus as $menu) {
    list($code, $name, $link, $target, $order) = $menu;
    $sql = " insert into `{$table_prefix}menu`
                set me_code = '{$code}',
                    me_name = '{$name}',
                    me_link = '{$link}',
                    me_target = '{$target}',
